...
describe('Collection.<methods>', function() {
    ...
    describe('#pluck()', function() {
        ...
        it('successful pluck with filter', function() {
            var col = new md.Collection({
                model: Model.User
            });
            var userA = new Model.User({
            ...
            col.addAll([userA, userB, userC, userD]);
            var result = col.pluck('id', function(mdl) {
                return _.startsWith(mdl.id(), 'B');
            });
            expect(result.length).to.equal(2);
            expect(result[0]).to.be.equal('Bar');
            expect(result[1]).to.be.equal('Baz');
        });
