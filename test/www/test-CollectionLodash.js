describe('Collection.<lodash>', function() {
  ...
  it('slice', function() {
    ...
    userC.active(true);
    col.add(userC);
    var slicedA = col.slice(0, 2);
    expect(slicedA).to.be.a('array');
    expect(slicedA.length).to.equal(2);
    expect(slicedA[0]).to.be.equal(userA);
    expect(slicedA[1]).to.be.equal(userB);
    var slicedB = col.slice(1, 3);
    expect(slicedB).to.be.a('array');
    expect(slicedB.length).to.equal(2);
    expect(slicedB[0]).to.be.equal(userB);
    expect(slicedB[1]).to.be.equal(userC);
  });
